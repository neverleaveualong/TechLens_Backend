...
export const PresetRepository = {
  async create(userId: number, p: PresetPayload): Promise<PresetRow> {
    const r = await pool.query(
      `INSERT INTO presets (
        user_tblkey, preset_name, applicant, start_date, end_date, description
      )
      VALUES ($1, $2, $3, $4, $5, $6)
      RETURNING *`,
      [
  ...
  async findById(userId: number, id: number): Promise<PresetRow | null> {
    const r = await pool.query(
      `SELECT * 
       FROM presets 
       WHERE preset_tblkey=$1 AND user_tblkey=$2`,
      [id, userId]
    );
